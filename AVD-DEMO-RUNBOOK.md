...
- **Live cost calculator** (step 2): pick the same Pizza Size you're about to choose in the portal and see real pricing — hourly rate, a ~20-minute demo estimate, and monthly-if-left-running — pulled live from the Azure Retail Prices API for `northcentralus`. Falls back to a small hardcoded rate table if that call is ever blocked, and says so on the page.
