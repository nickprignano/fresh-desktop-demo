...
- **Pizza-themed flavor pickers**, rendered natively by the Azure Portal's own deployment form (real ARM parameters with fun `allowedValues`, not a custom UI): Pizza Size (VM size: Personal Pan / Family Size / Large Party), Crust (Windows 11 plain vs. + Microsoft 365 Apps), and two Toppings (session capacity, clipboard/drive redirection). All default-selected so Review + Create needs zero clicks, but they're visible, real dropdowns you can change live on stage.
...
- **30-minute presentation countdown**: a fixed badge in the top-right corner of the page, visible on every step. It auto-starts the instant step 1's Deploy to Azure button is clicked (no extra click needed), and won't restart if you click Deploy twice. Manual **Start** / **Reset** buttons are also on the badge if you want to rehearse against the clock or restart it yourself. Color shifts green → amber at 15:00 remaining → pulsing red at 5:00, so you always have a passive read on your 30-minute budget without checking a separate clock.
...
5. **Open `deploy-avd-demo.html`** in your browser, signed in as `builder`. Everything from here is button clicks: step 1's **🚀 Deploy to Azure** button opens the Azure Portal deployment page directly against the hosted template, in a new tab, and starts the 30-minute presentation countdown in the top-right badge.

6. **Do one full dry run today**, not the morning of. Confirm the presentation countdown badge (top-right) auto-starts when you click Deploy, and that its color transitions (green → amber at 15:00 → pulsing red at 5:00) feel right against how long the real deployment actually takes. Click Deploy, fill in:
...
- Click **🚀 Deploy to Azure** on stage; the page opens the Azure Portal directly against the hosted template and starts the 30-minute presentation countdown badge (top-right) — a passive on-screen clock for you, no need to check your own watch. When the portal's Basics tab loads, point out the Pizza Size / Crust / Toppings dropdowns — defaults are already picked, or change one live for fun before hitting Review + Create.
- The instant you click **Create** in the portal, flip back to the tracker tab and hit **Start tracking my order**. Narrate each stage as it lights up.
- The tracker is a choreographed timer synced to typical deploy timing, not a live Azure feed — if the real deployment visibly finishes a stage early or late, just click the matching stage icon to resync. You're always in control on stage; nothing here can desync embarrassingly if you drive it.
- If you're worried about live provisioning time, consider deploying ~20 minutes before you're on, then just doing the login + narration live, and mentioning the deploy step is one click. Use the **8x rehearse** speed toggle beforehand to practice the narration without waiting 18 minutes each time. If you reset and re-rehearse, hit the countdown badge's **Reset** button too so it doesn't show stale time when you go live.
...
Step 4 on the page now handles this: type in the resource group name you used, click **Copy Command & Open Cloud Shell** once, and it both copies `az group delete --name <name> --yes --no-wait` to your clipboard and opens [Azure Cloud Shell](https://shell.azure.com) in a new tab (no local CLI needed) — just paste and hit enter. There's also a direct link to the Portal's Resource Groups view if you'd rather delete it by clicking through.
